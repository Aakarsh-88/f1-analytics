"use client";

import {
  CartesianGrid,
  Legend,
  Line,
  LineChart,
  ResponsiveContainer,
  Tooltip,
  XAxis,
  YAxis,
} from "recharts";

import type { ChampionshipProgressionPoint } from "@/types/standings";

const DRIVER_LINE_COLORS: Record<string, string> = {
  VER: "#3671C6",
  NOR: "#FF8000",
  LEC: "#E8002D",
};
const DEFAULT_LINE_COLOR = "#9B5DE5";

interface ProgressionChartProps {
  data: ChampionshipProgressionPoint[];
  driverCodes: string[];
}

export function ProgressionChart({ data, driverCodes }: ProgressionChartProps) {
  return (
    <ResponsiveContainer width="100%" height={320}>
      <LineChart data={data} margin={{ top: 8, right: 16, left: -8, bottom: 0 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="rgb(var(--surface-border))" vertical={false} />
        <XAxis
          dataKey="raceName"
          tick={{ fill: "rgb(var(--text-secondary))", fontSize: 11, fontFamily: "var(--font-mono)" }}
          axisLine={{ stroke: "rgb(var(--surface-border))" }}
          tickLine={false}
        />
        <YAxis
          tick={{ fill: "rgb(var(--text-secondary))", fontSize: 12, fontFamily: "var(--font-mono)" }}
          axisLine={false}
          tickLine={false}
        />
        <Tooltip
          contentStyle={{
            backgroundColor: "rgb(var(--surface-elevated))",
            color: "rgb(var(--text-primary))",
            border: "1px solid rgb(var(--surface-border))",
            borderRadius: 6,
            fontSize: 12,
            fontFamily: "var(--font-mono)",
          }}
          itemStyle={{ color: "rgb(var(--text-primary))" }}
        />
        <Legend wrapperStyle={{ fontSize: 12, fontFamily: "var(--font-mono)" }} />
        {driverCodes.map((code) => (
          <Line
            key={code}
            type="monotone"
            dataKey={code}
            stroke={DRIVER_LINE_COLORS[code] ?? DEFAULT_LINE_COLOR}
            strokeWidth={2}
            dot={{ r: 3 }}
          />
        ))}
      </LineChart>
    </ResponsiveContainer>
  );
}
